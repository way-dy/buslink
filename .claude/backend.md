...
- `passengers/{empNo}` (PIN 해시·노선 배정) + (선택, 2026-07-22) **`nfcUid`**(string|null — NFC 사원증 카드 UID. **정규화 계약=소문자 hex·구분자 없음**(`0453ce9a`), 정본 헬퍼 `src/lib/nfc.js normalizeNfcUid` / 서버 미러 `functions/index.js normalizeNfcUidServer`. 부재/null=카드 미등록. 등록은 협력사 포털(개별 등록 폼·수정 모달)에서만 — 승객 자가등록은 iOS 웹이 NFC 를 못 읽어 채택 안 함. `importEmployees` 는 `emp.nfcUid !== undefined` 일 때만 기록 → **엑셀 대량 등록이 기존 카드를 지우지 않음**)
