...
> 상태: **초안 — way 확정 전 구현 금지**. 세 가지 요구가 **같은 뿌리** 하나에 걸려 있어 묶었다.
...
| 단계 | 내용 | 크기 | 이 단계가 여는 것 |
|---|---|---|---|
| P0 | `boardStatic` 본인 확인(**완료·배포 대기**) | 소 | 무작위 사번 탑승 차단 |
| P1 | CF `passengerLogin` + 커스텀 토큰 + 클라 전환 | 중 | 승객 신원 생성. **회귀 표면 = 263명 전원의 로그인** |
| P2 | `boardStatic`·본인 문서 read/update 를 클레임 기준으로 | 소 | `pinHash` 를 클라에서 없앰 |
