...
- `[패턴]` **배포 안전 절차(2026-05-18, "특정 PC만" 규율 폐기)**: 이 저장소가 작업 정본 — 어느 PC든 `git pull`+`.env.local`(`.env.example` 참고)이면 빌드·배포 가능. 2026-05-18 prod 연쇄장애 실원인 = **ⓐ 카카오 키 단일**: `.env.local` 키가 `d464b4…`(전용키, 도메인 미등록)면 prod 지도 전체 사망 → **빌드 전 운영 공유키 `58bf34…` 확인 필수**. ⓑ ~~Auth 승인 도메인 누락~~ = **오진(2026-05-18 콘솔 실측 반증)**: `buslink-prod.web.app`·`firebaseapp.com`은 Firebase가 기본 자동 등록 — 누락 불가, 조치 불필요. prod 권한오류가 재발하면 도메인 아님 → 다른 원인(규칙/익명인증 흐름) 조사. 절차: 키확인→`npm run build`→`npm start` localhost `/p`·`/bus` 지도 확인→`firebase deploy --only hosting`→prod 재확인. **가설-재배포 금지**: 회귀 시 콘솔 Hosting 롤백 먼저, 재현은 localhost. (체크리스트 → tasks.md "배포 전 체크리스트")
