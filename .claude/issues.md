...
- `[해결]` **EmployeeApp `/p` 지도 흰화면 근본원인**(2026-05-18, 장기 미제 해결): `appWrap 100dvh→content flex:1→HomeTab flex:1→map flex:'0 0 55%'` 체인이라 카카오 맵 init 시점 컨테이너 0px → 카카오는 CSS와 달리 이후 높이 생겨도 자동 relayout 안 함 = 영구 흰화면. `/bus`는 지도 컨테이너 고정 `height:100vh`라 무관 → `/p`만 깨진 이유와 정확히 일치. 해결: `EmployeeApp.js` `<Map>`(HomeTab + 실시간 모달)에 `onCreate={map=>{map.relayout();setTimeout(()=>map.relayout(),300)}}`(무위험·idempotent). 플렉스/동적높이 컨테이너의 카카오 맵엔 항상 onCreate relayout 부착.
- `[패턴]` **배포 안전 절차(2026-05-18, "특정 PC만" 규율 폐기)**: 이 저장소가 작업 정본 — 어느 PC든 `git pull`+`.env.local`(`.env.example` 참고)이면 빌드·배포 가능. 2026-05-18 prod 연쇄장애 실원인 2가지(둘 다 PC 무관·재발 방지): ⓐ `.env.local` 카카오 키가 `d464b4…`(전용키, 도메인 미등록)면 prod 지도 전체 사망 → **빌드 전 운영 공유키 `58bf34…` 확인 필수**. ⓑ Firebase Auth 승인 도메인에 `buslink-prod.web.app`·`buslink-prod.firebaseapp.com` 없으면 prod `permission-denied`(localhost는 기본 허용이라 정상) → 콘솔 Authentication→Settings에서 1회 추가(코드·배포 무관). 절차: 키확인→`npm run build`→`npm start` localhost `/p`·`/bus` 지도 확인→`firebase deploy --only hosting`→prod 재확인. **가설-재배포 금지**: 회귀 시 콘솔 Hosting 롤백 먼저, 재현은 localhost. (체크리스트 → tasks.md "배포 전 체크리스트")
...
- `[패턴]` **노선 사전경로 `routes/{id}.routePath`**: `[{lat,lng}, ...]` plain number 배열(stops와 동일, GeoPoint 아님). AdminApp RoutesTab "경로 그리기" 모달이 수동 작성. 빈 배열/필드 없음=미설정. EmployeeApp/PassengerApp는 **유효좌표 ≥2면 사전경로, 아니면 `stops.map` 직선 폴백**(하위호환 계약 — 기존 노선 깨지지 않음, 신규/편집 노선부터 점진 적용). 실시간 경로진행·도착판정은 `lib/routeProgress.js`(투영 progress 기반)로 통일, 폴백 시 기존 노선순서 로직.
- `[패턴]` **GPS 콜드스타트 출발지 공백 = `watchPosition` 단독+`maximumAge:0,timeout:10000`+에러 console만**: 첫 측위가 10초 초과 시 TIMEOUT이 묻혀 출발 직후 수십초 `gps` 미기록. 해결 표준: ①`startGPS` 진입 즉시 `getCurrentPosition`(`maximumAge:30000,timeout:8000`) 1회 선발행(역행가드로 watch 첫콜백과 중복 차단) ②watch `timeout`30s·`maximumAge`10s 완화 ③TIMEOUT(code3) 1회 watch 재시작 가드(무한루프 방지)+`onGpsError` 상위 전파(권한 code1 포함). 상수 분리(`INITIAL_FIX_OPTS`/`WATCH_OPTS`). `sendGPS`/throttle/heartbeat/visitedStops 불변.
- `[패턴]` **권한·PWA설치 게이트는 `lib/usePermissions.js`+`components/PermissionGate.js`**(콜센터 `driver.html` getPermStates/배너/`beforeinstallprompt` 패턴의 React 정본·순수 브라우저API, Firebase import 금지). `beforeinstallprompt`는 등록 SW 필요 → `index.js`가 `firebase-messaging-sw.js` 1회 선등록(notifications.js 재등록과 idempotent). EmployeeApp홈·DriverApp 상단 마운트.
