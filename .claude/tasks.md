...
## 완료
- [x] **앱별 PWA 설치 아이콘(Route Family) 적용·배포** (2026-05-19, 커밋 `8b3e37e`, master push 완료) — `public/icons/` 6종(승객/기사/관제 svg+1024png) + manifest 3종(`manifest.json` 관제·기존수정 / `manifest-employee.json` scope `/p` / `manifest-driver.json`, purpose `any`만·maskable 미지정=Route 라인 가장자리 보존 의도) + `index.html`(favicon svg 우선+ico fallback, apple-touch admin-1024.png) + `src/lib/pwaManifest.js`(순수 DOM, idempotent) + EmployeeApp/DriverApp 마운트 1회 `applyAppManifest` 동적 교체(/p=승객·기사앱=기사·기본=관제). 격리=tracked 5(.gitignore·index.html·manifest.json·EmployeeApp·DriverApp)+신규4, App.js/PassengerApp/AdminApp/lib·ui 0줄. parse-only OK·신규 경고 0·번들 `main.80d0f31e.js`. **curl 실측(prod)**: manifest 3종 HTTP200·JSON OK(name/scope/start/icons 정확), 아이콘 6 HTTP200(svg=image/svg+xml,png=image/png), index.html `appkey=58bf34`(회귀X)·`rel="manifest"`·`apple-mobile-web-app-capable`·`autoload=false` 유지. 회귀 0. (로컬 래스터 도구 부재→manifest 아이콘 SVG 전략)
